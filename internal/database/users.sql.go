...
const getChirpIDByUserID = `-- name: GetChirpIDByUserID :many
...
func (q *Queries) GetChirpIDByUserID(ctx context.Context, id uuid.UUID) ([]uuid.UUID, error) {
	rows, err := q.db.QueryContext(ctx, getChirpIDByUserID, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
